...
| <a name="input_plan"></a> [plan](#input\_plan) | The IBM Cloud Logs plan to provision. Available plans: `lite`,`standard`, `standard-plus-metrics`. Currently,`standard-plus-metrics` is available for allowlisted accounts only. | `string` | `"standard"` | no |
