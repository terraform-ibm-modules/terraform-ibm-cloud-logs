...
locals {
  ...
  data_bucket_name    = var.cloud_logs_data_cos_bucket_name != null ? "${local.prefix}${var.cloud_logs_data_cos_bucket_name}" : null
  metrics_bucket_name = var.cloud_logs_metrics_cos_bucket_name != null ? "${local.prefix}${var.cloud_logs_metrics_cos_bucket_name}" : null
  cos_instance_guid   = module.existing_cos_instance_crn_parser.service_instance

  key_ring_name = var.cloud_logs_cos_key_ring_name != null ? "${local.prefix}${var.cloud_logs_cos_key_ring_name}" : null
  key_name      = var.cloud_logs_cos_key_name != null ? "${local.prefix}${var.cloud_logs_cos_key_name}" : null
